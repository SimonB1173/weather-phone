...
function buildMainMenuInto(twiml, activeLocationName) {
  ...
  say(gather, `${activeLocationName}. Press 1 for the forecast menu. Press 2 for hourly forecast. Press 3 for current weather.`);
...
function afterActionTwiml(unit = "C") {
  ...
  say(gather, `Press star to go back. Press pound to repeat. Press 5 to change location. ${unitText}`);
  twiml.hangup();
  return twiml;
}
...
function weatherCodeToText(code, isNight = false) {
  ...
  const map = { 45: "foggy", 48: "freezing fog", 51: "light drizzle", 53: "drizzle", 55: "heavy drizzle", 56: "light freezing drizzle", 57: "heavy freezing drizzle", 61: "light rain", 63: "rain", 65: "heavy rain", 66: "light freezing rain", 67: "heavy freezing rain", 71: "light snow", 73: "snow", 75: "heavy snow", 77: "snow grains", 80: "light showers", 81: "showers", 82: "heavy showers", 85: "light snow showers", 86: "heavy snow showers", 95: "thunderstorms", 96: "thunderstorms with hail", 99: "severe thunderstorms with hail" };
...
function classifyHourlyBucket(item, tz = "UTC") {
  ...
  else if (rainAmount > 0) condition = [80, 81, 82].includes(code) ? (rainAmount >= 1.5 ? "showers" : "light showers") : rainAmount >= 1.5 ? "rain" : "light rain";
  let precipTag = "dry";
...
function summarizeHourlyBlock(block, tz, unit = "C") {
  ...
  if (Math.abs(avgAppTemp - avgTemp) >= 3) parts.push(avgAppTemp < avgTemp ? `Wind chill near ${formatSignedTemp(avgAppTempC, unit)}.` : `Feels like ${formatSignedTemp(avgAppTempC, unit)}.`);
  if (maxRainChance >= 45 && totalRain <= 0.05 && totalSnow <= 0.02) parts.push(`${Math.round(maxRainChance)} percent chance of precipitation.`);
  if (totalRain > 0.05) parts.push(totalRain >= 3 ? "Rain at times." : "Light rain or a few showers.");
  if (totalSnow > 0.02) parts.push(totalSnow >= 2 ? "Snow at times." : "Light snow or flurries.");
  if (maxWind >= (unit === "F" ? 22 : 35) || maxGust >= (unit === "F" ? 34 : 55)) parts.push("Windy.");
  else if (maxWind >= (unit === "F" ? 11 : 18)) parts.push("A light breeze.");
  return parts.join(" ");
}
...
function buildSmartHourlyBlocks(items, tz = "UTC") {
  ...
  const blocks = [];
  let current = { key: classifyHourlyBucket(items[0], tz), start: items[0].time, end: items[0].time, items: [items[0]] };
  for (let i = 1; i < items.length; i++) {
    ...
    if (key === current.key) {
      ...
      blocks.push(current);
      current = { key, start: items[i].time, end: items[i].time, items: [items[i]] };
    }
  }
  blocks.push(current);
  return blocks;
...
async function fetchCanadianAlert(location) {
  ...
  try {
    const response = await axios.get(location.alertFeedUrl, { timeout: 8000, headers: { "User-Agent": "weather-line-canada/1.0" } });
    ...
    if (items.length > 0) {
      ...
      const noAlert = combined.includes("no watches or warnings in effect") || combined.includes("no alerts in effect") || combined.includes("no warning or watch in effect");
      if (!noAlert) alertValue = { title: summarizeAlertText(title, 120), description: summarizeAlertText(description, 260) };
    }
...
async function ecApiGet(url, params = {}) {
  const response = await axios.get(url, {
    params,
    timeout: 15000,
    headers: { "User-Agent": "weather-line-ec/3.0", Accept: "application/json" }
  });
...
async function findNearestEcCityPagePoint(location) {
  ...
  const data = await ecApiGet("https://api.weather.gc.ca/collections/citypageweather-realtime/items", { f: "json", lang: "en", bbox, limit: 250 });
  const features = safeArray(data?.features)
    .map((f) => {
      const coords = safeArray(f?.geometry?.coordinates);
      const lon = Number(coords[0]);
      const lat = Number(coords[1]);
      return { feature: f, distanceKm: haversineKm(location.latitude, location.longitude, lat, lon) };
    })
...
async function fetchEnvironmentCanadaCityPage(location) {
  ...
  const item = await ecApiGet(`https://api.weather.gc.ca/collections/citypageweather-realtime/items/${encodeURIComponent(pointId)}`, { f: "json", lang: "en" });
...
function ecIssuedSpeechLine(label, rawTimestamp) {
  const text = String(rawTimestamp || "").trim();
  if (!text) return `${label}.`;
  const iso = formatEcLocalIso(text);
  if (iso) return `${label} at ${formatLocalIsoTimeLabel(iso)}.`;
  return `${label} at ${text}.`;
}
...
function normalizeEcForecastPeriods(cityItem) {
  ...
  return {
    ...
    periods: periods.map((p, index) => ({
      ...
      summary: p?.abbreviatedForecast?.textSummary?.en || p?.textSummary?.en || p?.precipitation?.textSummary?.en || "",
      pop: safeNumber(p?.abbreviatedForecast?.pop?.value) ?? safeNumber(p?.pop?.value) ?? null,
      temperature: safeNumber(p?.temperatures?.temperature?.textSummary?.value) ?? safeNumber(p?.temperatures?.temperature?.value) ?? safeNumber(p?.temperature?.value) ?? null,
...
async function fetchEnvironmentCanadaData(location) {
  ...
  return { source: "environment-canada-citypage", item, current: normalizeEcCurrent(item), hourly: normalizeEcHourly(item), forecastPeriods: normalizeEcForecastPeriods(item) };
}

function ecCurrentWeatherSpeech(location, ecData, unit = "C") {
  const c = ecData.current;
  const parts = [`Current weather for ${placeLabel(location)}.`, ecIssuedSpeechLine("Issued by Environment Canada", c.issuedAt)];
  ...
  if (Number.isFinite(apparent) && Number.isFinite(c.temperatureC) && Math.round(tempValueForUnit(apparent, unit)) !== Math.round(tempValueForUnit(c.temperatureC, unit))) {
    parts.push(apparent < c.temperatureC ? `Wind chill ${formatSignedTemp(apparent, unit)}.` : `Feels like ${formatSignedTemp(apparent, unit)}.`);
  }
...
function ecHourlySpeech(location, ecData, hours = 12, unit = "C") {
  ...
  const future = rows.filter((row) => {
    ...
    return datePart > nowLocal.date || (datePart === nowLocal.date && hourPart > nowLocal.hour) || (datePart === nowLocal.date && hourPart === nowLocal.hour && minutePart >= nowLocal.minute);
  });
  const slice = future.slice(0, hours);
  if (!slice.length) return `I could not find hourly forecast data for ${placeLabel(location)}.`;
  const compact = slice.map((row) => ({ time: row.time, temp: row.temperature_2m, apparentTemp: row.apparent_temperature, rainChance: row.precipitation_probability, rain: row.rain, showers: row.showers, snow: row.snowfall, clouds: row.cloud_cover, wind: row.wind_speed_10m, gusts: row.wind_gusts_10m, direction: row.wind_direction_10m, code: row.weather_code }));
  const blocks = buildSmartHourlyBlocks(compact, location.timezone || "America/Toronto");
  const spoken = blocks.slice(0, 4).map((b) => summarizeHourlyBlock(b, location.timezone || "America/Toronto", unit)).join(" ");
  return [`Hourly forecast for ${placeLabel(location)}.`, ecIssuedSpeechLine("Issued by Environment Canada", ecData.forecastPeriods?.issuedAt || ecData.current?.issuedAt), spoken].join(" ");
}

function ecSingleForecastSpeech(location, ecData, index, unit = "C") {
  const periods = ecData.forecastPeriods?.periods || [];
  if (index < 0 || index >= periods.length) return `That forecast period is not available for ${placeLabel(location)}.`;
  const p = periods[index];
  const parts = [`${p.periodName}.`, ecIssuedSpeechLine("Issued by Environment Canada", ecData.forecastPeriods?.issuedAt)];
  if (p.summary) parts.push(p.summary);
  if (p.tempText && !p.summary.includes(p.tempText)) parts.push(p.tempText);
  if (Number.isFinite(p.temperature)) parts.push(`${/night|tonight/i.test(p.periodName) ? "Low" : "High"} ${formatForecastTempValue(p.temperature, unit)}.`);
  if (Number.isFinite(p.pop) && p.pop > 0 && p.pop < 100) parts.push(`${Math.round(p.pop)} percent chance of precipitation.`);
  if (p.windText) parts.push(p.windText);
  if (p.uvText) parts.push(p.uvText);
  return parts.join(" ");
}
...
function ecAllForecastSpeech(location, ecData, unit = "C") {
  ...
  const parts = [`Forecast for ${placeLabel(location)}.`, ecIssuedSpeechLine("Issued by Environment Canada", ecData.forecastPeriods?.issuedAt)];
  for (const p of periods) {
    let line = `${p.periodName}.`;
    if (p.summary) line += ` ${p.summary}`;
    if (p.tempText && !line.includes(p.tempText)) line += ` ${p.tempText}`;
    if (Number.isFinite(p.temperature)) line += ` ${/night|tonight/i.test(p.periodName) ? "Low" : "High"} ${formatForecastTempValue(p.temperature, unit)}.`;
    if (Number.isFinite(p.pop) && p.pop > 0 && p.pop < 100) line += ` ${Math.round(p.pop)} percent chance of precipitation.`;
    if (p.windText) line += ` ${p.windText}`;
    if (p.uvText) line += ` ${p.uvText}`;
...
function currentWeatherSpeech(location, forecast, unit = "C") {
  ...
  const parts = [`Current weather for ${placeLabel(location)}.`, `Retrieved at ${retrievedTimeLabelNow(location.timezone)}.`, `${describeCurrentWeatherSentence(c, location.timezone)}.`, `Temperature ${formatSignedTemp(c.temperature_2m, unit)} degrees.`];
  ...
  if (apparent !== null && Math.round(tempValueForUnit(apparent, unit)) !== Math.round(tempValueForUnit(Number(c.temperature_2m || 0), unit))) {
    parts.push(tempValueForUnit(apparent, unit) <= tempValueForUnit(Number(c.temperature_2m || 0), unit) - 3 ? `Wind chill ${formatSignedTemp(apparent, unit)}.` : `Feels like ${formatSignedTemp(apparent, unit)}.`);
  }
  ...
  if (speed >= (unit === "F" ? 9 : 15)) parts.push(gusts >= speed + (unit === "F" ? 6 : 10) ? `Wind ${direction} ${speed} ${speedUnitLabel(unit)}, gusting to ${gusts}.` : `Wind ${direction} ${speed} ${speedUnitLabel(unit)}.`);
  return parts.join(" ");
}
...
function nextHoursSpeech(location, forecast, hours = 6, unit = "C") {
  ...
  for (let i = 0; i < (h.time || []).length; i++) {
    ...
    const isFuture = datePart > nowLocal.date || (datePart === nowLocal.date && hourPart > nowLocal.hour) || (datePart === nowLocal.date && hourPart === nowLocal.hour && minutePart >= nowLocal.minute);
    if (isFuture) {
      items.push({ time: h.time[i], temp: h.temperature_2m?.[i], apparentTemp: h.apparent_temperature?.[i], rainChance: h.precipitation_probability?.[i], rain: h.rain?.[i], showers: h.showers?.[i], snow: h.snowfall?.[i], clouds: h.cloud_cover?.[i], wind: h.wind_speed_10m?.[i], gusts: h.wind_gusts_10m?.[i], direction: h.wind_direction_10m?.[i], code: h.weather_code?.[i] });
    }
...
function dailyForecastSpeech(location, forecast, index, unit = "C") {
  ...
  if (Number.isFinite(Number(d.precipitation_probability_max?.[index]))) parts.push(`${Math.round(Number(d.precipitation_probability_max[index]))} percent chance of precipitation.`);
  if (Number.isFinite(Number(d.wind_speed_10m_max?.[index]))) parts.push(`Wind up to ${Math.round(speedValueForUnit(d.wind_speed_10m_max[index], unit))} ${speedUnitLabel(unit)}.`);
  return parts.join(" ");
}
...
function sevenDayForecastSpeech(location, forecast, unit = "C") {
  ...
  const parts = [`Issued on ${issuedDateLabelToday(location.timezone)}.`, `Retrieved at ${retrievedTimeLabelNow(location.timezone)}.`, `Here is the 7 day forecast for ${placeLabel(location)}.`];
  ...
  const requestPromise = (async () => {
    try {
      ...
      if (location?.country === "CA") {
        ...
        const response = await axios.get("https://customer-api.open-meteo.com/v1/forecast", { params, timeout: 15000 });
        data = response.data;
...
async function forecastDayPromptTwiml(location, forecast) {
  ...
  if (location?.country === "CA") {
    const periods = forecast.forecastPeriods?.periods || [];
    const parts = ["For the full Environment Canada forecast, press 0."];
    for (let i = 0; i < Math.min(9, periods.length); i++) parts.push(`Press ${i + 1} for ${periods[i].periodName}.`);
  ...
  for (let i = 2; i < Math.min(7, dailyTimes.length); i++) parts.push(`Press ${i + 1} for ${fullDateLabel(dailyTimes[i], location.timezone)}.`);
  parts.push("Press star to go back to the previous menu.");
  say(gather, parts.join(" "));
  twiml.redirect({ method: "POST" }, "/forecast-menu");
  return twiml;
...
async function buildStateTwiml(req, state, { push = true } = {}) {
  ...
  if (state === "location-menu") return locationMenuTwiml({ allowBack: !!getActiveLocation(req), allowVoicemail: !getActiveLocation(req) });
  ...
  if (state === "forecast-menu") {
    ...
    if (!location) return locationMenuTwiml({ allowBack: false, allowVoicemail: true });
...
async function goBackOneMenu(req) {
  ...
  if (currentState === "playback" || currentState === "after-prompt") return buildStateTwiml(req, history[history.length - 1] || "location-menu", { push: false });
  if (history.length <= 1) return buildStateTwiml(req, "location-menu", { push: false });
  popMenuHistory(req);
  const updated = getMenuHistory(req);
  return buildStateTwiml(req, updated[updated.length - 1] || "location-menu", { push: false });
}
...
app.post("/voice", async (req, res) => {
  ...
  try {
    ...
    const gather = twiml.gather(gatherOptions("/set-location-choice", 7, 1));
    say(gather, "Welcome to Weather and Info Line. Press 1 for Montreal. 2 for Tosh. 3 for Laurentians. 4 for United States. Press 9 to leave a comment or suggestion.");
    twiml.redirect({ method: "POST" }, "/location-menu-prompt");
...
app.post("/forecast-day", async (req, res) => {
  ...
  try {
    ...
    if (selected < 0) {
      say(twiml, location.country === "CA" ? "I did not understand the forecast period." : "I did not understand the forecast day.");
      const forecastTwiml = await buildStateTwiml(req, "forecast-menu", { push: false });
      return res.type("text/xml").send(forecastTwiml.toString());
    }
    if (location.country === "CA") {
      const periodCount = forecast.forecastPeriods?.periods?.length || 0;
      if (selected >= periodCount) {
        say(twiml, "I did not understand the forecast period.");
        const forecastTwiml = await buildStateTwiml(req, "forecast-menu", { push: false });
        return res.type("text/xml").send(forecastTwiml.toString());
      }
    } else if (selected > 6) {
...
app.post("/handle-recording", (req, res) => {
  ...
  if (recordingUrl) {
    appendVoicemailRecord({ callSid, from, to, recordingUrl, recordingDuration, createdAt: new Date().toISOString(), source: "action" });
